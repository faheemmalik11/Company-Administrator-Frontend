...
const Employees: React.FC<Props> = ({ update_id }) => {
    ...
    const columns = [
        ...
        {
            name: 'Name',
            selector: (row: any) => <Link title='update' to={`/update_employee/${row.id}`}>{row?.name}</Link>
        },
        {
            name: 'City',
        ...
        {
            name: 'Action',
            cell: (row: any) => <button title='increment history' onClick={() => { incrementHandler(row.id) }}><svg
        ...
        {
            name: 'Delete',
            cell: (row: any) => <button title='delete Employee' onClick={() => { deleteEmployeeHandler(row.id) }}>
