...
const ItemTable: React.FC<Props> = ({ update_id }) => {
    ...
    const columns = [
        ...
        {
            name: 'Description',
            cell: (row: Iitems) => row.description
        },
        {
            name: 'Created at',
                        ...
                        itemData.map(item => {
                            return (
                                <tr className={((highlightedId == item.id) && isHighlighted) ? `bg-sky-500/100 border-2 border-green-600 ` : 'border-2'} key={item.id}>
                                    ...
                                    <td className="border-2">{item.created_at}</td>
                                    <td className="border-2">{item.quantity}</td>
                                    <td className="border-2">{item.store_id}</td>
                                    <td className="border-2">{item.category_id}</td>
                                    <td className="border-2">{item.updated_at}</td>
                                    <td className="border-2">
                                        <button title='update store data' onClick={() => {updateHandler(item.id) }}>
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-6 h-6">
                                                <path d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                                            </svg>
                                        </button>
                                    ...
                                    <td className="border-2">
                                        <button title='delete store data' onClick={() => {deleteHandler(item.id)}}>
                                            <svg className="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 16">
                                                <path d="M19 0H1a1 1 0 0 0-1 1v2a1 1 0 0 0 1 1h18a1 1 0 0 0 1-1V1a1 1 0 0 0-1-1ZM2 6v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V6H2Zm11 3a1 1 0 0 1-1 1H8a1 1 0 0 1-1-1V8a1 1 0 0 1 2 0h2a1 1 0 0 1 2 0v1Z" />
                                            </svg></button></td>
                                </tr>
